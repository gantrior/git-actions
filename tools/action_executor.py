...
def main():
    parser = argparse.ArgumentParser(
        description="Execute pending actions from daily files"
    )
    parser.add_argument(
        "--file",
        required=True,
        help="Path to daily action file(s) to execute (supports wildcards)"
    )
    parser.add_argument(
        "--allowlist",
    ...
    parser.add_argument(
        "--commit",
        action="store_true",
        help="Commit results to git after each action execution"
    )
    parser.add_argument(
        "--no-commit",
        action="store_true",
        help="Do not commit results (useful for testing)"
    )
            ...
            if report.results:
                ...
                for result in report.results:
                    if result.status == "success":
                        ...
                        print(f"  ⏭️  {result.action_id} ({result.action_name}): Skipped - {result.error}")
