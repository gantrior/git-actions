...
from tools.editor import ActionUpdate, update_action_entry
from tools.parser import parse_daily_file
from tools.validator import get_current_environment, load_allowlist
...
def execute_action_script(
    script_path: str, action_data: Dict[str, Any], timeout: int = 300
) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"status": status, "outputs": outputs if status == "success" else {}, "error": error}

    except subprocess.TimeoutExpired:
        return {"status": "error", "outputs": {}, "error": f"Timeout after {timeout}s"}
    except FileNotFoundError:
        return {"status": "error", "outputs": {}, "error": f"Script not found: {script_path}"}
    except Exception as e:
        return {"status": "error", "outputs": {}, "error": f"Unexpected error: {str(e)}"}


def commit_action_result(file_path: str, action_id: str, message: Optional[str] = None) -> None:
...
def execute_actions_from_file(
    file_path: str, allowlist_path: str, commit: bool = False
) -> ExecutionReport:
    ...
    for action in pending_actions:
        ...
        action_data = {"action": action.name, "version": action.version, "inputs": action.inputs}

        # Execute the action script
        start_time = time.time()
        script_result = execute_action_script(
            script_path=entry.script, action_data=action_data, timeout=entry.timeout
        )
        duration = time.time() - start_time
        ...
        meta = {"executedAt": executed_at, "runId": run_id}
        if result.error:
            meta["error"] = result.error

        update = ActionUpdate(
            action_id=action.id, check_box=True, outputs=result.outputs, meta=meta
        )
